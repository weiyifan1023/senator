...
class RewardModel(nn.Module):
    ...
    def forward(self,
        ...
        if self.config.model_type == "llama" or self.config.model_type == "baichuan" or self.config.model_type == "aquila":
            kwargs = dict()
        else:
            kwargs = dict(head_mask=head_mask)
        if self.config.model_type == "baichuan" or self.config.model_type == "aquila":
            transformer_outputs = self.rwtransformer.model(#.model
                input_ids,
    ...
    def forward_value(self,
        ...
        # if hidden_states.device != input_ids.device:
        #     hidden_states = hidden_states.to(input_ids.device)
        # self.v_head = self.v_head.to(hidden_states.device)
